  ...
  */
  ...
  let restaurantFinderApplication = {
    ...
    findAvailableRestaurants: function (numberOfPeople) {
      // Complete here
    let availableRestaurants = this.restaurants.filter(r => (r.totalSeats - r.numberOfCustomers) >= numberOfPeople);
    ...
    findRestaurantServingDish: function (dishName) {
      // Complete here
      let restaurantServingDish = this.restaurants.filter(r => r.menu.includes(dishName))
      let restaurantNameServingDish =  restaurantServingDish.map(r => r.name)
      return restaurantNameServingDish
    },

    countNumberOfRestaurantsInArea: function (area) {
      // Complete here
      let flterRestaurants = this.restaurants.filter(r => r.address.area === area)
      return flterRestaurants.length;
    },
  };
